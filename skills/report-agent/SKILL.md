---
name: report-agent
description: 把访谈、问卷、PDF、Word、Excel、CSV 等异构素材整理成面向管理层的研究报告：论据先清洗核验，再由 Writer 撰写，多个 Judge 分维度评测并驱动改写，达标后交付；长期写作记忆默认启用，从用户反馈中沉淀写作要求用于后续评测，可由用户明确关闭。当用户要求生成或修改调研洞察、战略分析、复盘报告、高管汇报，或要求开启、关闭、整理报告写作记忆时使用。
---

# 调研洞察汇报报告生成 · 研究报告专家团 V4
...
按 [Writer 调用与续写](references/writer-orchestration.md) 委派 `report-writer`，由它完整读取 [writing-instructions.md](references/writing-instructions.md)，按其中的证据边界、三段结构、洞察和表达要求写作。
...
用户新增、替换或纠正素材/论据时，先按 [evidence-orchestration.md](references/evidence-orchestration.md) 更新论据并说明影响，再向用户确认是否据此修改报告；同意后按修改范围处理，不同意则保留新数据、报告不动。用户明确要求只更新论据或仅检查时不改报告。事实更新不作为写作偏好存入 Memory，修改后的报告不沿用旧评分。
