...
`writerAgentId` 只保存宿主实际返回的 agent_id，team.writerName 保存其真实消息地址；字段未知时为 null。team.assignments 按 [团队调用契约](native-agent-contracts.md) 保存派发与回传的对应关系，恢复时先核验当前团队成员是否仍可达。不得用 task_id 或 Writer 自报文本代替 agent_id，也不在进入 Loop 时清空已确认的记录。
...
不要把全部原始 Judge 输出直接塞给 Writer。**用脚本生成**，不手工提炼——brief 是 Writer
能看到的全部评测信息，若由主 Agent 凭理解归纳，同一份 Judgment 每次会得到宽严不一的改写
指令，信息屏障也就失去意义：

```text
python3 "<资源目录>/report/revision_brief.py" \
  --plan "<评测与改写记录/resolution-plan.json>" \
  --best "<历史最佳的各维结果文件>" [--best 可重复] \
  --best-version "<RN>" \
  --user-requirements "<用户确认的本轮要求>" \
  [--rejected "<被拒候选结果>" --rejected-parent "<其父版本结果>"] \
  --output "<评测与改写记录/revision-briefs/<待生成RN>.json>"
```

脚本输出 `REVISION_BRIEF_COMPLETED` 与各类条目数；返回 `REVISION_BRIEF_FAILED` 时按评测
不完整处理，不要自己补一份 brief。文件名使用待生成候选的 RN（如生成 R1 的要求存为
R1.json）。脚本按冻结 Plan 分流，不引入新标准：
