...
用户反馈按 [Writer 调用与续写](references/writer-orchestration.md#反馈分流) 处理：重大修改（如通篇改写、改变分析方向）启动新一轮 Report Loop，小型修改直接 Rewrite；主 Agent 判断并简短说明，用户明确指定的方式优先。修订或 Loop 完成并核验报告后，Memory 已开启时按 [memory-orchestration.md](references/memory-orchestration.md) 委派 `report-team-memory-v2 operation=capture`，再交付；关闭时直接交付。
