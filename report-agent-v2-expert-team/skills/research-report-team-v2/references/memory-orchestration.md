...
- 本契约中的 Memory 专指 `report-team-memory-v2` 管理的 L0 Writing Episode、L1 Atom Memory 和 L2B Memory Rubrics。
- Memory 是持久化用户级功能，默认启用。用户明确要求查询、关闭或重新开启时，主 Agent 委派 `report-team-memory-v2 operation=settings`；关闭不会删除已有记忆。
...
- 固定顺序：按 [Writer 反馈分流](writer-orchestration.md#反馈分流) 完成直接 Rewrite 或新 Loop → 主 Agent 核验报告 → 委派 `report-team-memory-v2 operation=capture` → 再交付或总结。重大修改也只针对本次用户反馈 Capture 一次，不把 Loop 的自动改写或 Judge 意见当成新反馈。
