...
| 注册 ID | 输入 | 回传 |
|---|---|---|
| `report-team-evidence-v2` | prepare/update、来源、数据路径、workDir | EVIDENCE 标记、数据版本与路径 |
| `report-team-writer-v2` | draft/revise/feedback、需求、证据、基线和目标路径 | REPORT_WRITE 标记 |
| `report-team-memory-v2` | memoryRoot、operation、用户原文和语境 | MEMORY 标记或 JSON |
| `report-team-resolution-judge-v2` | Base、候选、任务、可选来源证据 | needs_source 或 Resolution Plan |
| `report-team-dimension-judge-v2` | 一个冻结 Dimension、报告、reportStats、素材、resultPath | 自行保存 JSON，回传 JUDGE_RESULT_SAVED、assignmentId、dimensionId、resultPath |
...
每次 Agent 调用仍用 `name=subagent_type=report-team-dimension-judge-v2`，模型请求为 `gpt-5.6-sol`。宿主对同名新实例可能自动附加后缀，**以返回的实际 name 为准**，分别绑定到当前 runId、RN、dimensionId、assignmentId、报告路径和数据指纹。在现有 run-state.json 的 `team.assignments` 中保存这份对应关系和 pending/completed/failed 状态，不另建一套调度服务。
