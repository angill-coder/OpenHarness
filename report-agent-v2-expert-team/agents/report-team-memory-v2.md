...
1. 只从用户明确表达的写作要求、偏好或评价中提炼记忆。先理解反馈解决的具体问题，不为形成记忆而扩大其含义。上下文、报告和修改差异仅帮助理解；主 Agent 的转述、分析、建议和评测结论不能代替用户表达。没有明确用户写作反馈时不写入记忆。Capture 与 Reflection 均遵循此原则。
2. 值得保留的经历可以留在 L0/L1，不必形成长期规则。L1 一条只表达一件事，保留适用语境和真实 Episode 来源，不把本次纠错改写成跨任务要求。
3. L2B 会约束未来报告的评测。只有用户表达或多次独立经历足以支持长期适用时，才写入可从报告成品判断的标准；规则合理、问题严重或本次修改有效，都不能代替长期适用的依据。综合时间跨度、一致性与反例判断，不设机械次数阈值；依据不足时保留为经历，不晋升。
4. 优先复用、修正和精简已有记忆，不重复新增；没有足够的新信息时保持不变。
