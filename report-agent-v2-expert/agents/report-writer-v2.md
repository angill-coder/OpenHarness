...
- `mode=draft`：用户原始请求、已确认的三项输入及用户原文、共享论据表与原始素材路径、dataVersion/dataSha256、写作指令路径、V0 目标路径。完整理解论据并按需回查素材，不能只依赖主 Agent 的摘要；不读取 Memory 或等待 Judge 才写初稿。若附有 `baselineReportPath`，这是同一报告更新资料后重新评测：以旧报告为基础按新论据及确认需求生成新 V0，不默认从零重写，不把旧评分或反馈当成本轮冻结标准，不覆盖基线。
...
- `mode=feedback`：用户新反馈原文、相关对话与资料变化、当前交付报告基线、未占用的历史版本目标路径、dataVersion/dataSha256。直接按反馈改写并保存新版本，不自行启动 Loop；主 Agent 核验后更新终稿、记录版本和另行委派记忆。
...
4. 所有模式只写指定的 `历史版本/<报告主题>-vN.md` 新文件，目标已存在时停止，不覆盖历史稿或基线；主 Agent 核验后更新终稿。
