...
test("follow-up feedback routes by impact while preserving explicit user choices", () => {
  ...
  assert.match(contract, /使用 `AskUserQuestion` 确认是否据此修改报告/u);
  assert.match(contract, /等待用户同意后，再按/u);
  assert.match(contract, /不同意则保留新数据、报告不动/u);
  assert.match(contract, /确认前不派发 Writer\/Judge/u);
  assert.match(contract, /纯写作反馈仍直接按整体影响分流/u);
  assert.match(contract, /且没有其他修改或重评要求.*结束/u);
  assert.doesNotMatch(contract, /让用户选择.*Loop.*Rewrite/u);
  const skill = read("skills/research-report-agent-v2/SKILL.md");
  ...
  assert.match(writer, /数据更新引起的报告修改.*取得用户确认/u);
  assert.match(skill, /向用户确认是否据此修改报告.*同意后按修改范围处理/u);
