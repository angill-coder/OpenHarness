...
- 更新时附 `previousPath`、`changeRequest` 和用户更正原文；用户无需指出新增/修改的文件，按 [素材变化识别](references/source-changes.md) 自动扫描。用户直接给出的事实补充由主 Agent 原样保存为本次来源文件，并明确其“用户陈述、未独立核验”的性质。

只有受众或写法改变、素材未变时，无需重新清洗。缺少更新所需的来源或无法判断哪个文件被替换时，返回具体问题给主 Agent，不猜测。

## 执行

1. 核对路径与输入范围，按 [素材变化识别](references/source-changes.md) 用脚本比较内容指纹。无可信基线时核验全部当前来源及旧论据；有基线时先读旧论据表，只解析新增或修改的文件，并核验删除来源、受影响 Evidence 和必要交叉证据。目录是合法输入，中文路径与空格按原样传递，不自行替换分隔符。不要把旧论据表、素材清单、输出目录或解析中间文件当成新增独立来源。
...
5. 校验 JSON 格式、唯一 ID、来源、数值/单位/分母和变更范围。按 [输出契约](references/output-contract.md) 先保存并验证本轮候选，再更新共享文件并重新读取确认。没有论据变化则返回旧路径，不创建空版本；本轮快照仍保留。全部变化处理成功后才由脚本确认素材清单，解析或发布失败不更新清单。
...
只更新指定的共享论据文件及同目录的 `素材清单.json`，并在 `workDir` 保存本轮产物；不修改原始素材、历史报告快照、Judge 或 Memory。共享文件格式错误、所属项目不符或整理期间被其他任务改动时，拒绝覆盖，返回具体问题，不强行重建。
