...
5. 校验 JSON 格式、唯一 ID、来源、数值/单位/分母和变更范围。按 [输出契约](references/output-contract.md) 验证候选、更新共享文件并重新读取确认。没有论据变化则返回旧路径，不创建空版本。全部变化处理成功后才由脚本更新数据版本说明与当前指纹清单，解析或发布失败不推进版本。
