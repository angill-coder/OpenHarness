...
class FrontendContractTest(unittest.TestCase):
    ...
    def test_rubrics_loop_uses_reading_and_annotation_layout(self):
        ...
        self.assertIn("正在对feedback进行分类", source)
        self.assertIn("state.classificationRunning=true", source)
        self.assertIn("classification-running", source)
        self.assertNotIn("分流", source + html)
        self.assertIn('id="memoryStore"', html)
        self.assertIn('id="memoryUser"', html)
        self.assertIn("/api/rubrics-loop/memory", source)
        self.assertIn("memory_actions:memoryActions", source)
        self.assertIn("仅记录，待观察（L0）", source)
        self.assertIn("保存为稳定偏好（L1 + L2）", source)
        self.assertIn("Writing Episode", source)
        self.assertIn("选中的报告原文", source)
        self.assertIn("item.externalSourceId", source)
        self.assertIn("function historyValidationBuckets(group)", source)
        self.assertIn("未验证 · 累计 ${count} 轮", source)
        self.assertIn("已验证待决策 · ${date} · ${count} 轮", source)
        self.assertIn("已决策 · ${date} · ${count} 轮", source)
        self.assertIn("无需验证 · Memory ${count} 轮", source)
        self.assertIn("选择本次要验证的未验证轮次", source)
        self.assertIn("data-validation-round", source)
        self.assertIn("验证所选轮次", source)
        self.assertIn("selected_batch_ids:retrying?null:state.validationSelection", source)
        self.assertIn("included_batch_ids", source)
        self.assertIn("已完成 · Memory", source)
        self.assertIn("查看本轮结果", source)
        ...
        self.assertIn(".classification-running", styles)
    ...
    def test_rubrics_loop_candidate_review_is_readable_and_diffable(self):
        ...
        self.assertIn("operationFeedbackIds.has(item.feedback_id)", source)
        self.assertIn("candidate.operations||[]", source)
        self.assertIn("update_existing_checks", source)
        self.assertIn("const chronological=(group.iterations||[]).slice().sort", source)
        self.assertIn("[iteration.batch_id,index+1]", source)
        self.assertIn("const chronological=iterations.slice().sort", source)
        self.assertIn("return {round:index+1", source)
        self.assertIn("validated_pending_decision", source)
        self.assertIn("已验证待决策", source)
        self.assertIn("decided:`已决策", source)
        self.assertIn("['adopted','rejected'].includes(candidate?.status)", source)
        self.assertIn("const defaultOpen=['unvalidated','validating','validated_pending_decision']", source)
        self.assertNotIn("待验证草案：已累计", source)
        self.assertIn("function renderCandidateChanges", source)
        self.assertIn("/api/rubrics-loop/feedback/confirm-and-optimize", source)
        self.assertIn("Memory 与 Rubrics Optimizer 并行处理中", source)
        self.assertIn("确认分类并同时处理", source)
        ...
        self.assertNotIn('id="rubricFileInput"', html)
        self.assertNotIn("/api/rubric/import", source)
        self.assertNotIn("$('rubricFileInput')", source)
        self.assertNotIn("更换当前 Session Rubric", html)
        ...
        self.assertNotIn("只替换当前 Session 的 Rubric", html)
