...
function renderHistory(){
  ...
  if(!session){
    ...
    $('historyGroups').innerHTML=sessions.map(item=>{const running=item.active_experiment_count?`<em>${esc(item.active_experiment_count)} 个验证实验运行中</em>`:'';return `<details class="history-session"><summary><span><b>${esc(item.session_id)}</b><small>Rubrics ${esc(item.rubric_version||'未知版本')} · ${esc(item.iteration_count)} 轮 · 最近 ${esc(timeLabel(item.updated_at))}</small></span>${running}</summary><div>${item.active_draft?`<div class="active-draft-summary"><b>待验证草案：已累计 ${esc(item.active_draft.revision_count)} 轮</b><span>涉及 ${esc((item.active_draft.touched_check_ids||[]).join('、')||'无 Check 变更')} · ${esc(statusLabel(item.active_draft.status))}</span></div>`:''}${historyGroupsHtml(item.groups,item.session_id)}</div></details>`;}).join('');bindHistoryActions();return;
  }
  const groups=state.iterationHistory?.groups||[];$('historyHint').textContent=`当前查看：${session.session_id}。按父 Rubrics 版本归档，选择一轮可打开候选 Rubrics 审核。`;$('historyEmpty').classList.toggle('hidden',!!groups.length);
  ...
  $('historyGroups').innerHTML=draftHtml+historyGroupsHtml(groups,session.session_id,selectedCandidateId);bindHistoryActions();
}
