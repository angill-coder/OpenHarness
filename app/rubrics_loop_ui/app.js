...
function setupModelSelectors(){
  const backends=state.config.llm_backends||['api','codex','workbuddy'];['optimizerBackend','skillBackend','judgeBackend'].forEach(id=>{$(id).innerHTML=backends.map(value=>`<option value="${value}">${value==='workbuddy'?'WB CLI':value==='codex'?'Codex CLI':'API'}</option>`).join('');});
  ...
  $('optimizerBackend').value=state.config.optimizer_llm_backend||'api';$('skillBackend').value=state.config.optimizer_llm_backend||'api';$('judgeBackend').value=state.config.judge_llm_backend||'api';
  ['optimizer','skill','judge'].forEach(prefix=>{const backend=$(prefix+'Backend');backend.onchange=()=>renderModelSelect(prefix);renderModelSelect(prefix);});
  ['optimizerEffort','skillEffort','judgeEffort'].forEach(id=>{$(id).innerHTML=(state.config.codex_reasoning_efforts||[]).map(value=>`<option>${esc(value)}</option>`).join('');});
}
function renderModelSelect(prefix){const backend=$(prefix+'Backend').value,select=$(prefix+'Model');let models,def;if(backend==='api'){models=state.config.api_models;def=prefix==='judge'?state.config.judge_api_model:state.config.optimizer_api_model;}else if(backend==='codex'){models=state.config.codex_models;def=prefix==='judge'?state.config.judge_codex_model:state.config.optimizer_codex_model;}else{models=state.config.evaluation_models;def=prefix==='judge'?state.config.judge_wb_model:state.config.optimizer_wb_model;}select.innerHTML=(models||[]).map(value=>`<option value="${esc(value)}">${esc(value)}</option>`).join('');if(def&&[...select.options].some(option=>option.value===def))select.value=def;const effortLabel=$(prefix==='optimizer'?'effortLabel':prefix+'EffortLabel');if(effortLabel)effortLabel.classList.toggle('hidden',backend!=='codex');}
...
$('prepareExperiment').onclick=()=>{const experimentSessionId=state.experiment?.candidate_id===state.candidate?.candidate_id&&state.experiment?.experiment_session_id;if(experimentSessionId){location.href='/?id='+encodeURIComponent(experimentSessionId);return;}$('experimentPanel').classList.remove('hidden');$('experimentPanel').scrollIntoView({behavior:'smooth'});};
function renderExperiment(){const result=state.experiment;if(!result)return;$('experimentResult').textContent=JSON.stringify({status:result.status,experiment_session_id:result.experiment_session_id,feedback_results:result.feedback_results,comparison:result.comparison,error:result.error},null,2);if(result.experiment_session_id){$('openExperiment').href='/?id='+encodeURIComponent(result.experiment_session_id);$('openExperiment').classList.remove('hidden');}else $('openExperiment').classList.add('hidden');const canAdopt=result.status==='completed'&&['validated','awaiting_review'].includes(state.candidate?.status);$('adoptCandidate').classList.toggle('hidden',!canAdopt);$('startExperiment').textContent=result.status==='failed'?'重试验证实验':'创建隔离验证实验';$('startExperiment').disabled=!['created','failed'].includes(result.status);}
