...
def _research_requirement_contract(
    ...
    if any(k in text for k in ("重点分布", "重点素材", "材料重点", "汇报的重点")):
        interaction_fields.append(
            "材料重点分布（重点主题/结论/素材及预期权重）"
        )
    if any(k in text for k in ("篇幅", "页数", "字数", "长度")):
        interaction_fields.append(
            "报告篇幅（页数或字数；页数按每页不超过约 1000 个中文可见字符折算）"
        )
    ...
    lines = [
        ...
        "- **篇幅换算**：优先遵守用户明确给出的字数；用户给页数时按每页不超过约 1000 个中文可见字符折算，表格单元格文字计入、Markdown 标记和空白不计；用户未指定时默认 3 页以内 / 3000 字以内。篇幅是上限，不为凑满而注水。",
