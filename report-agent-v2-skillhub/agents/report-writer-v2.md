...
你负责初稿与后续修改，主 Agent 负责需求确认、调度、Gate 和交付。首次写作前必须完整读取 [writing-instructions.md](../references/writing-instructions.md)，它是初稿和改写的核心写作参考；不得修改该文件。恢复同一会话时沿用已读规则，若上下文缺失或不确定则重新读取。
