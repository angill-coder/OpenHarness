...
class ReportLoopRuntime:
    def __init__(
        ...
        # 记忆由 report-memory-agent 维护在 ReportAgentMemory/；这里只读 L2B。
        self.memory_provider = memory_provider or MemoryRubricProvider()
        ...
        self._explicit_run_dirs: dict[str, str] = {}
    ...
    def _run_dir(self, run_id: str) -> Path:
        ...
        # 报告目录内的 Loop 目录优先（见 skills/research-report-loop/references/workspace-and-delivery.md）；
        # 未指定时退回插件数据目录，保持既有运行不受影响。
        explicit = self._explicit_run_dirs.get(value) or self._read_run_index().get(value)
        if explicit:
    ...
    def _run_index_path(self) -> Path:
        return self.data_dir / "run-index.json"

    def _read_run_index(self) -> dict[str, str]:
    ...
    def _register_run_dir(self, run_id: str, directory: Path) -> None:
        """记录 runId 到报告目录内 Loop 目录的映射，供跨进程恢复使用。"""
        resolved = str(Path(directory).resolve())
        self._explicit_run_dirs[run_id] = resolved
        index = self._read_run_index()
        index[run_id] = resolved
        self.data_dir.mkdir(parents=True, exist_ok=True, mode=0o700)
        self._atomic_json(self._run_index_path(), index)

    def run_directory(self, run_id: str) -> Path:
        """Return the validated directory for a Report Loop run."""
        return self._run_dir(run_id)

    def _records_dir(self, run_id: str) -> Path:
        """评测与改写记录目录。

        报告目录内布局把记录收在 `评测与改写记录/` 下，避免与用户可见的
        候选报告混在一起；插件数据目录布局沿用扁平结构。
        """
        base = self._run_dir(run_id)
        records = base / RECORDS_DIR_NAME
        return records if records.is_dir() else base
    ...
    def _save(self, run: dict[str, Any]) -> None:
        run["updatedAt"] = _now()
        records = self._records_dir(run["id"])
        self._atomic_json(records / "state.json", run)
        if records.name == RECORDS_DIR_NAME:
            # 规范命名的运行状态，便于人工回查
            self._atomic_json(records / "run-state.json", run)

    def _event(self, run_id: str, event: str, payload: dict[str, Any]) -> None:
        path = self._records_dir(run_id) / "events.jsonl"
        path.parent.mkdir(parents=True, exist_ok=True)
    ...
    def _load(self, run_id: str) -> dict[str, Any]:
        path = self._records_dir(run_id) / "state.json"
        if not path.is_file():
    ...
    def start(
        ...
        *,
        ...
        skillVersion: str = "research-report-loop/1.0.0",
        loopDir: str | None = None,
        reportDir: str | None = None,
        reportTopic: str | None = None,
        reportId: str | None = None,
        dataVersion: str | None = None,
        dataSha256: str | None = None,
        roundRequest: str | None = None,
    ) -> dict[str, Any]:
        ...
        if loopDir:
            # 报告目录内的 Loop 目录：报告/.report-agent/<报告标识>/loop-NNN-时间戳/
            directory = Path(loopDir).expanduser().resolve()
            self._register_run_dir(run_id, directory)
        else:
            directory = self._run_dir(run_id)
        with self._lock:
            directory.mkdir(parents=True, exist_ok=True, mode=0o700)
            records = directory / RECORDS_DIR_NAME if loopDir else directory
            records.mkdir(parents=True, exist_ok=True, mode=0o700)
            self._atomic_json(records / "base_rubric.json", base_rubric)
            self._atomic_json(records / "memory_rubrics.json", memory_snapshot)
            self._atomic_json(records / "rubric_resolution_plan.json", resolution_plan)
            if loopDir:
                # 规范命名的冻结计划，便于人工回查
                self._atomic_json(records / "resolution-plan.json", resolution_plan)
            frozen_rubric = records / "compiled_rubric.json"
            frozen_rubric.write_text(rubric_raw, encoding="utf-8")
            if structured_data is not None:
                self._atomic_json(records / "structured_data.json", structured_data)
            if loopDir and str(roundRequest or "").strip():
                (directory / ROUND_REQUEST_FILE).write_text(
                    str(roundRequest).strip() + "\n", encoding="utf-8"
                )
            created = _now()
            run = {
                "schemaVersion": 2,
                ...
                "structuredDataPath": "structured_data.json" if structured_data is not None else None,                "skillVersion": str(skillVersion or ""),
                "reportDir": str(Path(reportDir).expanduser().resolve()) if reportDir else None,
                "reportTopic": str(reportTopic or "").strip() or None,
                "reportId": str(reportId or "").strip() or None,
                "loopDir": str(directory) if loopDir else None,
                "dataVersion": str(dataVersion or "").strip() or None,
                "dataSha256": str(dataSha256 or "").strip() or None,
    ...
    def submit(
        ...
        with self._lock:
            ...
            records_dir = self._records_dir(runId)
            rubric = json.loads((records_dir / "compiled_rubric.json").read_text(encoding="utf-8"))
            structured_data = None
            if run.get("structuredDataPath"):
                structured_data = json.loads(
                    (records_dir / run["structuredDataPath"]).read_text(encoding="utf-8")
                )
            case = {
                "case_id": runId,
        ...
        judgment = {
            ...
            "judgeModelPinned": judge_settings_are_pinned(actual_settings),
            "createdAt": _now(),
        }
        ...
        with self._lock:
            ...
            # 内部候选 RN 每轮 Loop 独立从 R0 初稿开始递增；RN 与交付版本 vN
            # 没有数字对应关系（见 skills/research-report-loop/references/workspace-and-delivery.md）。
            candidate_name = f"R{int(version[1:]) - 1}"
            if records_dir.name == RECORDS_DIR_NAME:
                report_path = run_dir / CANDIDATES_DIR_NAME / f"{candidate_name}.md"
            else:
            ...
            if records_dir.name == RECORDS_DIR_NAME:
                # 规范布局：judgments/RN.json、revision-briefs/RN.json
                self._atomic_json(
                    records_dir / "judgments" / f"{candidate_name}.json", judgment
                )
            else:
                self._atomic_json(run_dir / "judgments" / f"{version}.json", judgment)
            self._stop(run)
            brief = self._revision_brief(run, rubric, item)
            if records_dir.name == RECORDS_DIR_NAME:
                self._atomic_json(
                    records_dir / "revision-briefs" / f"{candidate_name}.json", brief
                )
            else:
    ...
    def finish(self, *, runId: str, reason: str | None = None) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            best = self._revision(run, run["currentVersion"])
            judge_settings_used = JudgeSettings(
                provider=best["judgment"]["judgeProvider"],
                model=best["judgment"]["judgeModel"],
                effort=best["judgment"]["judgeEffort"],
            )
            fallback_used = bool(best["judgment"].get("judgeFallbackUsed"))
            # Scores are only comparable to history when the pinned Judge model
            # actually scored this run. An override or a fallback to the host
            # model shifts the baseline, so say so explicitly rather than
            # letting the number look interchangeable.
            scores_comparable = (
                judge_settings_are_pinned(judge_settings_used) and not fallback_used
            )
            return {
                "status": "completed",
                "runId": runId,
                ...
                "judgeFallbackUsed": fallback_used,
                "judgeModelPinned": judge_settings_are_pinned(judge_settings_used),
                "scoresComparableToHistory": scores_comparable,
            }
