...
from .judge_model import (
    CODEX_JUDGE_EFFORT,
    CODEX_JUDGE_MODEL,
    SUPPORTED_EFFORTS as _SUPPORTED_EFFORTS,
)

DEFAULT_MODEL = CODEX_JUDGE_MODEL
DEFAULT_EFFORT = CODEX_JUDGE_EFFORT
...
def call_codex(
    ...
    *,
    ...
    """Run a stateless, read-only Codex Judge and return its final text."""
    ...
    with tempfile.TemporaryDirectory(prefix="report-loop-codex-") as temporary:
        ...
        if not text:
            raise CodexError("Codex Judge 返回空结果")
        return text
