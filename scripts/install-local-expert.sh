...
# build.py 产出 dist/<name>-<version>.zip，内部顶层目录为 <name>/。
# 解到临时目录再安装，保证装进去的就是打包校验过的那份内容。
ARCHIVE="$ROOT/dist/$NAME-$VERSION.zip"
if [ ! -f "$ARCHIVE" ]; then
  echo "Built archive not found: $ARCHIVE" >&2
  echo "Run without --skip-build, or run: npm run build" >&2
  exit 1
fi
UNPACKED=$(mktemp -d)
trap 'rm -rf "$UNPACKED"' EXIT
(cd "$UNPACKED" && unzip -q "$ARCHIVE")
PACKAGE_DIR="$UNPACKED/$NAME"
if [ ! -d "$PACKAGE_DIR" ]; then
  echo "Archive layout unexpected: missing $NAME/ inside $ARCHIVE" >&2
  exit 1
fi
...
for legacy in report-agent-v3 research-report-team report-loop; do
...
const LEGACY_NAMES = ["report-agent-v3", "research-report-team", "report-loop"].filter((value) => value !== name);
