...
新任务将正式报告、历史版本与 `Agent运行记录/` 内部文件分开保存；既有实验恢复时沿用原路径，不搬迁旧数据。长期记忆独立保存在用户主目录下可见的 `ReportAgentMemory/`，两种形态共用，不依赖 WorkBuddy 原生 Memory；其中 `MEMORY.md` 保存设置、索引与 L2B，`L0-episodes/` 和 `L1-atoms/` 保存来源，`history/` 保存修改历史。仓库不包含真实报告、实验日志或用户记忆。
