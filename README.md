...
`install-local-expert.sh` 解压构建产物再安装，保证装进去的就是打包校验过的那份；同时退役此前同一专家的旧名目录（`report-agent-v3` 等），避免市场里双列。安装后重启 WorkBuddy，在「专家 → 我的专家」找「报告写作专家团」，确认显示主理人与 5 位成员——6 个角色不等于只能运行 6 个实例。
