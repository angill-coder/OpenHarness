...
test("uses the cross-platform home directory rather than Documents or WorkBuddy", () => {
  const dataDir = path.join(os.homedir(), ".research-report-memory-v2-mvp");
  assert.equal(resolveUserShortcut(dataDir), path.join(os.homedir(), "Research Report Memory"));
  assert.equal(resolveLegacyDocumentsShortcut(dataDir), path.join(os.homedir(), "Documents", "Research Report Memory"));
  assert.equal(resolveUserShortcut(path.join(os.tmpdir(), "custom-memory")), undefined);
});

test("creates an idempotent user shortcut to the L2/L3 repository", async () => {
