...
import { isMemoryEnabled, resolveMemoryRoot } from "./lib/memory-settings.ts";
import { classifyWritingFeedback } from "./lib/relevance.ts";
import { ReportLoopLauncher } from "./lib/report-loop-launcher.ts";

const MEMORY_AGENT = "report-memory-agent";
...
function resolveStateDir() {
  ...
  return (configured || path.join(os.homedir(), ".research-report-loop", "capture-hook-state"))
...
async function handleReportLoopJobWrite(toolName, toolInput, sessionId) {
  ...
  if (payload?.schemaVersion !== 2 || !payload?.draftArtifactPath || !payload?.outputPath) return undefined;
...
async function onPrompt(input) {
  ...
  const memoryEnabled = isMemoryEnabled(resolveMemoryRoot());
  ...
  const systemMessage = state.capturePending
    ? [
      ...
      state.pendingKind === "context"
        ? "无需为纯背景纠正改写报告；直接通过 Agent/Task 委派 report-memory-agent 执行 operation=capture。"
        : "先落实当前报告修改，再通过 Agent/Task 委派 report-memory-agent 执行 operation=capture。",
      "这是独立的报告写作记忆（L0 Writing Episode / L1 Atom / L2B Rubrics），以纯 Markdown 存放在主目录的 ReportAgentMemory/，不是 WorkBuddy MEMORY.md 或项目日志。",
      "不要由主理人直接读写记忆文件，也不要把 WorkBuddy 原生 Memory 当作替代写入。",
    ].join(" ")
    : "";
...
async function onStop(input) {
  ...
  if (!isMemoryEnabled(resolveMemoryRoot())) {
...
main().catch((error) => {
  process.stderr.write(`[report-agent-v3] capture hook failed: ${error?.message || error}\n`);
  process.exitCode = 1;
});
