...
test("Guard requires WB Memory Sub-agent recall before drafting", (t) => {
  ...
  runHook(stateDir, "post-tool", {
    session_id,
    tool_name: "mcp__research-report-memory-v2-0821__writing_memory_recall",
...
test("Guard requires WB Memory Sub-agent capture before delivery", (t) => {
  ...
  assert.equal(runHook(stateDir, "pre-tool", {
    ...
    tool_input: { toolName: "mcp__research-report-memory-v2-0821__writing_memory_capture", params: {} },
  }).continue, true);
...
test("Feedback after a delivered report requires rewrite before Curator capture", (t) => {
  const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "memory-v2-guard-"));
  t.after(() => fs.rmSync(stateDir, { recursive: true, force: true }));
  const session_id = "session-rewrite-first";
  runHook(stateDir, "prompt", { session_id, prompt: "帮我写一份研究报告" });
  completeRecall(stateDir, session_id);
  runHook(stateDir, "post-tool", {
    session_id,
    tool_name: "mcp__research-report-loop__report_loop_finish",
    tool_response: { status: "ok", bestArtifactPath: "/tmp/report.md" },
  });
  const feedback = runHook(stateDir, "prompt", {
    session_id,
    prompt: "以后我的报告必须采用 IMRD，摘要后补充背景和方法",
  });
  ...
  const rewritten = runHook(stateDir, "post-tool", {
    session_id,
    tool_name: "Edit",
    tool_input: { file_path: "/tmp/report.md" },
    tool_response: { status: "ok" },
  });
  assert.match(rewritten.systemMessage, /operation=capture/u);
  assert.equal(runHook(stateDir, "pre-tool", {
    session_id,
    tool_name: "Agent",
    tool_input: { subagent_type: "research-report-memory-curator", operation: "capture" },
