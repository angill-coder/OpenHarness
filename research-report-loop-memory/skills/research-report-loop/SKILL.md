...
1. **汇报背景**：这份汇报给谁看、什么场合？
2. **摘要观点假设（hypothesis）**：完成素材解析后，提炼 1–3 条可被素材验证、反驳或修正的完整判断，主动请用户确认、修改或补充。每条 hypothesis 应能在验证后直接转化为摘要核心观点，清楚说明判断对象、方向性结论及关键关系、原因或对比；不能只写成主题、关键词或短标题。观点必须来自素材，证据不足时保留不确定性。用户也可以直接提出自己的 hypothesis。
3. **重点素材**：哪些文件或材料质量更高、应优先采用？

Hypothesis 的交互顺序固定为：先在普通回复中按编号完整展示 1–3 条观点，再调用 `AskUserQuestion` 做简短确认。工具问题只写“是否采用以上 hypothesis？”之类的短句，选项只保留“确认采用”“需要调整”等短标签；不得把 hypothesis 正文重复塞进问题、选项或描述字段，也不得为了适配交互控件而压缩观点。
