...
const WRITING_RECALL_ENABLED = process.env.RESEARCH_REPORT_MEMORY_WRITING_RECALL !== "off";
...
const FILE_MUTATION_TOOL_PATTERN = /^(?:Write|Edit|MultiEdit|NotebookEdit|ApplyPatch|apply_patch)$/u;
...
const SKILL_NAME = "research-report-loop";
...
const SKILL_FILE_PATTERN = /(?:^|\/)skills\/research-report(?:-loop)?\/(?:SKILL\.md|references\/)/iu;
...
function resolveStateDir() {
  ...
  const memoryRoot = process.env.RESEARCH_REPORT_MEMORY_V2_0821_DIR?.trim();
...
function activateSkill(state, source) {
  ...
  if (WRITING_RECALL_ENABLED && firstActivation && !state.revisionFeedbackPending) {
...
async function onPrompt(input) {
  ...
  const wasAwaitingRecall = WRITING_RECALL_ENABLED
    && state.researchReportActive && state.recallPending;
  if (!WRITING_RECALL_ENABLED) {
  ...
  if (!CANCEL_REPORT_PATTERN.test(prompt) && !explicitMemoryManagement
    ...
    } else if (WRITING_RECALL_ENABLED && NEW_REPORT_PATTERN.test(prompt)) {
      state.revisionFeedbackPending = false;
      state.reportProduced = false;
      state.recallPending = true;
      state.recallDueNow = false;
  ...
  if (feedback.relevant) {
    ...
    if (revisionFeedback || state.reportProduced) {
      state.revisionFeedbackPending = true;
  ...
  const reminders = [];
  if (WRITING_RECALL_ENABLED && state.researchReportActive && state.recallPending) {
    reminders.push(
      state.recallDueNow
  ...
  if (state.researchReportActive && state.capturePending) {
    reminders.push(
      `本轮包含可能需要写作记忆处理的反馈：${state.pendingFeedback}。${state.rewritePending ? "先直接修改当前报告文件，不要重跑 Report Loop；确认修改成功后，" : ""}再通过 Agent/Task 委派 ${MEMORY_AGENT} 独立判断 ignore/pending/store 并调用 ${CAPTURE_TOOL}；主 Agent 不得直接调用 Memory MCP。之后才能交付或总结。`,
      "本轮必须使用 operation=capture；即使反馈与旧 Memory 冲突，也不得改走 manage 或在 Capture 期间单独调用 forget。",
      "写作反馈不得修改已安装 Skill、Base Rubric、插件代码、~/.workbuddy/MEMORY.md 或项目 .workbuddy/memory；MCP 失败时也不得用这些文件兜底。",
    );
  }
  printJson({
    continue: true,
...
async function onPreTool(input) {
  ...
  if (WRITING_RECALL_ENABLED && state.recallPending && FILE_MUTATION_TOOL_PATTERN.test(toolName)) {
    reason = `即将开始写入报告，但尚未完成写作记忆读取。请先委派 ${MEMORY_AGENT}；收到 MEMORY_RECALL_COMPLETED 或明确的 MEMORY_RECALL_FAILED 后，再重试本次写入。`;
  } else if (state.rewritePending && memoryAgentInvocation) {
    reason = `当前是用户反馈修订，尚未确认当前报告已修改。请先直接修改报告文件，不要运行 Report Loop；修改成功后再以 operation=capture 委派 ${MEMORY_AGENT}。`;
...
async function onPostTool(input) {
  ...
  if (successful && detectSkillActivation(toolName, input.tool_input)) {
    ...
    changed = true;
    if (WRITING_RECALL_ENABLED && firstActivation && state.recallPending) {
      activationMessage =
  ...
  if (WRITING_RECALL_ENABLED && successful && INTAKE_TOOL_PATTERN.test(toolName)
    && state.researchReportActive && state.recallPending) {
    state.recallDueNow = true;
    changed = true;
    activationMessage =
  ...
  if (successful && state.rewritePending && FILE_MUTATION_TOOL_PATTERN.test(toolName)) {
    state.rewritePending = false;
    changed = true;
    activationMessage = `当前报告修改已完成。现在通过 Agent/Task 以 operation=capture 委派 ${MEMORY_AGENT}；不要直接调用 Memory MCP，也不要修改 Skill、Rubric 或全局 Memory。`;
  }

  if (successful && (/report_loop_finish$/iu.test(toolName) || /^(?:present_files)$/iu.test(toolName))) {
    state.reportProduced = true;
    changed = true;
  }

  if (WRITING_RECALL_ENABLED && memoryAgentInvocation && MEMORY_RECALL_MARKER.test(serializedResult)) {
    state.researchReportActive = true;
    state.skillActivated = true;
  ...
  if (WRITING_RECALL_ENABLED && memoryAgentInvocation && state.recallPending
...
async function onStop(input) {
  ...
  if (WRITING_RECALL_ENABLED && state.researchReportActive && state.recallPending && state.recallDueNow) {
    missing.push(
  ...
  if (state.researchReportActive && !state.recallPending && state.capturePending) {
    missing.push(state.rewritePending
      ? `先直接修改当前报告文件，不要重跑 Report Loop；修改成功后再委派 ${MEMORY_AGENT} Capture：${state.pendingFeedback || "用户本轮写作反馈"}`
      : `通过 Agent/Task 委派 ${MEMORY_AGENT} 处理本轮写作反馈并调用 ${CAPTURE_TOOL}（可用 ignore/pending/store）：${state.pendingFeedback || "用户本轮写作反馈"}`);
  }
...
main().catch((error) => {
  ...
  process.stderr.write(`[research-report-memory-v2-0821] hook failed: ${error?.message || error}\n`);
  process.exitCode = 1;
});
