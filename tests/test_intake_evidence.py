...
from report_loop.runner import JobError, load_job
...
class IntakeEvidenceTests(unittest.TestCase):
    ...
    def test_judge_model_is_absent_unless_explicitly_requested(self):
        """No override in the Job means the pinned model is used."""
        loaded = load_job(self.write_job(self.job()))
        self.assertNotIn("judgeModel", loaded)
        self.assertNotIn("judgeEffort", loaded)

    def test_explicit_judge_model_is_carried_through(self):
        payload = self.job()
        payload["judgeModel"] = "better-model-2027"
        payload["judgeEffort"] = "high"

        loaded = load_job(self.write_job(payload))

        self.assertEqual(loaded["judgeModel"], "better-model-2027")
        self.assertEqual(loaded["judgeEffort"], "high")

    def test_blank_judge_model_is_treated_as_absent(self):
        payload = self.job()
        payload["judgeModel"] = "   "
    ...
    def test_unsupported_judge_effort_is_rejected_at_load(self):
        payload = self.job()
        payload["judgeEffort"] = "turbo"
