...
from report_loop.core.workbuddy_cli import (
...
class WorkBuddyCliStdinTests(unittest.TestCase):
    ...
    def test_discovery_uses_host_provided_codebuddy_and_node_on_windows(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                ...
                patch("report_loop.core.workbuddy_cli.os.name", "nt"),
                patch("report_loop.core.workbuddy_cli.Path", NATIVE_PATH),
            ):
    ...
    def test_windows_desktop_fallback_remains_available_without_node(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                ...
                patch("report_loop.core.workbuddy_cli.os.name", "nt"),
                patch("report_loop.core.workbuddy_cli.Path", NATIVE_PATH),
                patch("report_loop.core.workbuddy_cli.shutil.which", return_value=None),
            ):
    ...
    def test_standalone_windows_cli_precedes_desktop_fallback(self):
        ...
        with (
            ...
            patch("report_loop.core.workbuddy_cli.os.name", "nt"),
            patch("report_loop.core.workbuddy_cli.shutil.which", side_effect=which),
            patch(
                "report_loop.core.workbuddy_cli._windows_desktop_command",
    ...
    def test_long_prompt_is_sent_via_stdin_not_argv(self):
        ...
        with (
            patch(
                "report_loop.core.workbuddy_cli.discover_command",
                return_value=("workbuddy",),
            ),
            patch(
                "report_loop.core.workbuddy_cli.build_environment",
                return_value={},
            ),
            patch(
                "report_loop.core.workbuddy_cli.parse_stream_output",
                return_value="OK",
            ),
            patch(
                "report_loop.core.workbuddy_cli.subprocess.run",
