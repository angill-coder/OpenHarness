...
- 默认启用写作记忆，可明确要求关闭。与专家版共用用户主目录下的 `ReportAgentMemory/`：`MEMORY.md` 保存设置、revision 和生效 L2B；`memory-history.md` 记录实际变更，按需读取；`L0-episodes/`、`L1-atoms/` 保存来源。不维护全量索引，不依赖 WorkBuddy 原生 Memory，不自动清空旧记忆。
