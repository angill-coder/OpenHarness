...
V0 保存后立即在本轮工作区创建 `.report-agent/state/run-state.json`；下列路径用于新任务，恢复已有任务时沿用已记录的原路径：
...
不要把全部原始 Judge 输出直接塞给 Rewriter。主 Agent从历史最佳版本的 Judgment 生成 `.report-agent/state/revision-briefs/<version>.json`：
